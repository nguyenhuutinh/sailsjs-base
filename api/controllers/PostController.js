...
module.exports = {
    ...
    get: (req, res) => {
        ...
        if (req.query.slug) {
            Posts.findOne({ slug: req.query.slug }).populate('screenshot').populate('versions').populate('categories',{select : 'id'}).exec((err, data) => {
            ...
            Posts.findOne({ id: req.params.id }).populate('screenshot').populate('versions').populate('categories', {select : 'id'}).exec((err, data) => {
