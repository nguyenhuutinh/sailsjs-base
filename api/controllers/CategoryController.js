...
module.exports = {
    ...
    uploadCatScreenshot: function (req, res) {
        ...
        req.file('attach').upload({
            ...
            Category.findOne({id: req.params.id}).exec(function (err, cat) {
                console.log(cat)
    ...
    update: (req, res) => {
        ...
        let newData = { name: req.body.name, previewText: req.body.previewText };
      
        Category.update(req.params.id , newData).exec((err) => {
            if (err) res.send(500, { error: 'db error' });
